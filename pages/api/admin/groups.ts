...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        if (req.method === 'GET') {
            // List all groups
            const response = await axiosInstance.get(`${ckanUrl}/api/3/action/group_list`, {
                headers: { Authorization: SYSADMIN_API_TOKEN },
                params: { all_fields: true }
            });
            return res.status(200).json({ success: true, groups: response.data.result });
            ...
            const response = await axiosInstance.post(`${ckanUrl}/api/3/action/group_create`, {
            ...
            return res.status(201).json({ success: true, group: response.data.result });
            ...
            await axiosInstance.post(`${ckanUrl}/api/3/action/group_delete`, {
                id
            }, {
                headers: {
                    Authorization: SYSADMIN_API_TOKEN,
